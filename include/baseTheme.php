...
function draw($toolContent, $menuTypeID, $tool_css = null, $head_content = null, $body_action = null, $hideLeftNav = null, $perso_tool_content = null) {
	...
	if (is_array ( $toolArr )) {
		...
		if ($menuTypeID == 2) {
			$t->set_var ( 'THIRD_BAR_TEXT', q(ellipsize(htmlspecialchars_decode($intitule, ENT_QUOTES), 64)) );
		...
		if (isset ( $currentCourseID ) && ! $courseHome) {
			...
			$t->set_var ( 'BREAD_TEXT', q(htmlspecialchars_decode($intitule, ENT_QUOTES)) );
			if ($statut == 10)
			...
			$t->set_var ( 'BREAD_TEXT', htmlspecialchars_decode($intitule, ENT_QUOTES));
			$t->set_var ( 'BREAD_ARROW', '&#187;' );
...
if (isset($_SERVER['HTTP_REFERER']) && strlen($_SERVER['HTTP_REFERER']) > 0) {
	...
	if (startsWith($reff, "http://"))
		$reff = substr($reff, 7);
	else if (startsWith($reff, "https://"))
		$reff = substr($reff, 8);
	if (!startsWith($reff, $_SERVER['HTTP_HOST'])) {
