...
if (isset($post_id) && $post_id) {
	...
	$sql .= "WHERE (f.forum_id = '".mysql_real_escape_string($forum)."') AND (t.topic_id = '".mysql_real_escape_string($topic)."')";
	...
	$sql .= "WHERE (f.forum_id = '".mysql_real_escape_string($forum)."') AND (t.topic_id = '".mysql_real_escape_string($topic)."') AND (t.forum_id = f.forum_id)";	
}
...
if (isset($_POST['submit']) && $_POST['submit']) {
	...
	$sql = "UPDATE topics SET topic_replies = topic_replies+1, topic_last_post_id = $this_post, topic_time = '$time' 
		WHERE topic_id = '".mysql_real_escape_string($topic)."'";
	$result = db_query($sql, $currentCourseID);
	...
	if (isset($quote) && $quote) {
		$sql = "SELECT pt.post_text, p.post_time, u.username 
			...
			WHERE p.post_id = '".mysql_real_escape_string($post)."' AND pt.post_id = p.post_id";
